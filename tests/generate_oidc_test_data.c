			   ...
			   const char *issuer)
{
	...
	if (json_object_set_new(config, "openid_configuration_url",
				json_string(openid_configuration_url))) {
		goto cleanup;
	}

	if (json_object_set_new(config, "user_name_claim",
				json_string(user_name_claim))) {
		goto cleanup;
	}

	if (json_object_set_new(required_claims, "aud",
	...
	if (json_object_set_new(config, "minimum_jwk_refresh_time",
				json_integer(0))) {
		goto cleanup;
	}
...
cjose_jws_t *create_jws(cjose_jwk_t *key, json_t *header, json_t *claims)
{
	...
	cjose_jws_t *jws = cjose_jws_sign(key, header,
					  (const uint8_t *)claims_str,
		    ...
		    json_int_t not_before, json_int_t expires)
{
	...
	json_t *claims = create_claims(audience, issuer, issued_at, not_before,
				       expires, user_name);
			   ...
			   const char *user_name_claim)
{
	...
	retval = snprintf(oidc_config_file, sizeof(oidc_config_file),
			  "%s/oidc.json", output_folder);

	retval = snprintf(openid_configuration_file,
	...
	retval = snprintf(keys_file, sizeof(keys_file), "%s/keys.json",
			  output_folder);
	if (retval < 0 || retval > sizeof(openid_configuration_file)) {
		exit(1);
	}
	retval = snprintf(openid_configuration_uri,
			  sizeof(openid_configuration_uri),
			  "file://localhost%s", openid_configuration_file);
	if (retval < 0 || retval > sizeof(openid_configuration_file)) {
		exit(1);
	}

	retval = snprintf(keys_uri, sizeof(keys_uri), "file://localhost%s",
			  keys_file);
	if (retval < 0 || retval > sizeof(openid_configuration_file)) {
		exit(1);
	}

	json_t *oidc_config = create_oidc_config(openid_configuration_uri,
						 "preferred_username",
