...
#define MSS_ADJUST(x)          \
	x += TCP_HEADER_SIZE + \
	     ((ws->proto == AF_INET) ? (IP_HEADER_SIZE) : (IPV6_HEADER_SIZE))
...
static struct ev_loop *worker_loop;
...
static void term_sig_watcher_cb(struct ev_loop *loop, ev_signal *w,
				int revents);
...
static void link_mtu_set(worker_st *ws, struct dtls_st *dtls, unsigned int mtu);
 ...
 */
static int tls_pull_timeout(gnutls_transport_ptr_t ptr, unsigned int ms)
{
	int ret;
	int fd = (long)ptr;
	struct pollfd pfd;
...
#if !defined(CAPTURE_LATENCY_SUPPORT)
static ssize_t dtls_pull(gnutls_transport_ptr_t ptr, void *data, size_t size)
{
	dtls_transport_ptr *p = ptr;
...
static int dtls_pull_timeout(gnutls_transport_ptr_t ptr, unsigned int ms)
{
	int ret;
	dtls_transport_ptr *p = ptr;
	int fd = p->fd;
...
static ssize_t dtls_push(gnutls_transport_ptr_t ptr, const void *data,
			 size_t size)
{
	dtls_transport_ptr *p = ptr;
...
int get_psk_key(gnutls_session_t session, const char *username,
		gnutls_datum_t *key)
{
...
static int setup_dtls_psk_keys(gnutls_session_t session, struct worker_st *ws)
{
	...
	if (!ws->session) {
		oclog(ws, LOG_ERR,
		      "cannot setup PSK keys without an encrypted CSTP channel");
		return -1;
	}
	...
	ret = gnutls_prf(ws->session, PSK_LABEL_SIZE, PSK_LABEL, 0, 0, 0,
			 PSK_KEY_SIZE, (char *)ws->master_secret);
	if (ret < 0) {
...
static int setup_legacy_dtls_keys(gnutls_session_t session,
				  struct worker_st *ws)
{
	int ret;
	gnutls_datum_t master = { ws->master_secret,
				  sizeof(ws->master_secret) };
	...
	ret = gnutls_priority_set_direct(
		session, ws->req.selected_ciphersuite->gnutls_name, NULL);
	if (ret < 0) {
	...
	ret = gnutls_session_set_premaster(
		session, GNUTLS_SERVER,
		ws->req.selected_ciphersuite->gnutls_version,
		ws->req.selected_ciphersuite->gnutls_kx,
		ws->req.selected_ciphersuite->gnutls_cipher,
...
static int setup_dtls_connection(struct worker_st *ws, struct dtls_st *dtls)
{
...
#if defined(CAPTURE_LATENCY_SUPPORT)
	int ts_socket_opt = SOF_TIMESTAMPING_RX_SOFTWARE |
	...
	if (ws->req.use_psk && ws->session) {
		...
		oclog(ws, LOG_INFO,
		      "setting up legacy DTLS (resumption) connection");
...
#if defined(CAPTURE_LATENCY_SUPPORT)
	ret = setsockopt(dtls->dtls_tptr.fd, SOL_SOCKET, SO_TIMESTAMPING,
			 &ts_socket_opt, sizeof(ts_socket_opt));
	if (ret == -1)
		oclog(ws, LOG_DEBUG,
		      "setsockopt(UDP, SO_TIMESTAMPING), failed.");
#endif
			...
			unsigned int discon_reason)
{
	...
	if (ret < 0) {
		e = errno;
		oclog(ws, LOG_WARNING, "error in sending BAN IP message: %s",
		      strerror(e));
		return;
	}

	ret = recv_msg(ws, ws->cmd_fd, CMD_BAN_IP_REPLY, (void *)&reply,
...
static void send_stats_to_secmod(worker_st *ws, time_t now,
				 unsigned int discon_reason)
{
	...
	if (sd >= 0) {
		...
		msg.remote_ip = human_addr2((void *)&ws->remote_addr,
					    ws->remote_addr_len, buf,
					    sizeof(buf), 0);
...

{
	...
	while (pos < msg->size) {
		...
		if (type == 0) { /* server name ext */
			SKIP16(pos, msg->size);
			SKIP16(pos,
			       msg->size); /* we don't support anything but a single name */
			...
			if (hsize == 0 || hsize + pos > msg->size ||
			    hsize > sizeof(ws->buffer) - 1) {
				oclog(ws, LOG_DEBUG,
			...
			oclog(ws, LOG_DEBUG, "client requested hostname: %s",
			      (char *)ws->buffer);

			ws->vhost = find_vhost(ws->vconfig, (char *)ws->buffer);
			if (ws->vhost->name &&
			    strcasecmp(ws->vhost->name, (char *)ws->buffer) !=
				    0) {
				oclog(ws, LOG_INFO,
				      "client requested hostname %s does not match known vhost",
				      (char *)ws->buffer);
			}
 ...
 */
static void peek_client_hello(struct worker_st *ws, gnutls_session_t session,
			      int fd)
{
	...
	if (size <
	    TLS_RECORD_HEADER + TLS_HANDSHAKE_HEADER + HANDSHAKE_SESSION_ID_POS)
		goto fallback;

	msg.data = ws->buffer + TLS_RECORD_HEADER + TLS_HANDSHAKE_HEADER;
	msg.size = size - (TLS_RECORD_HEADER + TLS_HANDSHAKE_HEADER);
	hello_hook_func(session, GNUTLS_HANDSHAKE_CLIENT_HELLO, GNUTLS_HOOK_PRE,
			1, &msg);
...
restart:
	...
	if (parser.method == HTTP_GET) {
		...
		if (fn == NULL) {
			oclog(ws, LOG_HTTP_DEBUG, "unexpected URL %s",
			      ws->req.url);
			response_404(ws, parser.http_minor);
			goto finish;
		}
...
static void data_mtu_send(worker_st *ws, unsigned int mtu)
{
 ...
 */
static void data_mtu_set(worker_st *ws, struct dtls_st *dtls, unsigned int mtu)
{
	if (dtls->dtls_session) {
...
static int mtu_not_ok(worker_st *ws, struct dtls_st *dtls)
{
	...
	if (ws->proto == AF_INET) {
		const unsigned int min = MIN_MTU(ws);
		...
		if (getsockopt(dtls->dtls_tptr.fd, IPPROTO_IPV6, IPV6_PATHMTU,
			       &mtuinfo, &len) < 0 ||
		    mtuinfo.ip6m_mtu < 1280) {
			oclog(ws, LOG_INFO,
		...
		if (mtuinfo.ip6m_mtu > ws->adv_link_mtu) {
			oclog(ws, LOG_INFO,
			      "the discovered IPv6 MTU (%u) is larger than the advertised (%u); disabling MTU discovery",
			      (unsigned int)mtuinfo.ip6m_mtu, ws->adv_link_mtu);
			return 0;
		}
#else
		link_mtu_set(ws, dtls, MIN_MTU(ws));
 ...
 */
static void mtu_discovery_init(worker_st *ws, struct dtls_st *dtls,
			       unsigned int mtu)
{
	const unsigned int min = MIN_MTU(ws);
...
static void mtu_ok(worker_st *ws, struct dtls_st *dtls)
{
	unsigned int c;
			  ...
			  unsigned int dpd)
{
	...
	if (DTLS_ACTIVE(ws)->udp_state == UP_ACTIVE &&
		...
		unsigned int data_mtu = DATA_MTU(ws, ws->link_mtu);
...
static void set_no_delay(worker_st *ws, int fd)
{
	...
	if (ret == -1)
		oclog(ws, LOG_DEBUG, "setsockopt(TCP_NODELAY) to %x, failed.",
		      (unsigned int)flag);
}
...
static int dtls_mainloop(worker_st *ws, struct dtls_st *dtls,
			 struct timespec *tnow)
{
	...
	case UP_INACTIVE:
		...
		oclog(ws, LOG_TRANSFER_DEBUG, "received %d byte(s) (DTLS)",
		      ret);
		...
		if (ret == GNUTLS_E_REHANDSHAKE) {
			...
			DTLS_FATAL_ERR_CMD(
				ret, exit_worker_reason(ws, REASON_ERROR));
			...
			if (bandwidth_update(&ws->b_rx,
					     data.size - CSTP_DTLS_OVERHEAD,
					     tnow) != 0) {
				ret = parse_dtls_data(ws, data.data, data.size,
						      tnow->tv_sec);
				if (ret < 0) {
					oclog(ws, LOG_INFO,
			...
			oclog(ws, LOG_TRANSFER_DEBUG, "no data received (%d)",
			      ret);
...
hsk_restart:
		...
		if (ret == GNUTLS_E_LARGE_PACKET) {
			...
			oclog(ws, LOG_DEBUG, "Main DTLS session %d active",
			      ws->dtls_active_session);
			session_info_send(ws);
		}
...
static int tls_mainloop(struct worker_st *ws, struct timespec *tnow)
{
	...
	if (ret == 0) { /* disconnect */
		...
		oclog(ws, LOG_TRANSFER_DEBUG, "received %d byte(s) (TLS)",
		      data.size);

		if (bandwidth_update(&ws->b_rx, data.size - 8, tnow) != 0) {
			ret = parse_cstp_data(ws, data.data, data.size,
					      tnow->tv_sec);
			if (ret < 0) {
...
static int tun_mainloop(struct worker_st *ws, struct timespec *tnow)
{
	...
	if (l < 0) {
		...
		if (e != EAGAIN && e != EINTR) {
			oclog(ws, LOG_ERR,
			      "received corrupt data from tun (%d): %s", l,
			      strerror(e));
			return -1;
		}
	...
	if (DTLS_ACTIVE(ws)->udp_state == UP_ACTIVE &&
		...
		if (ret > 0 && ret < l) {
			...
			if (ws->cstp_selected_comp) {
				if (ws->cstp_selected_comp->id ==
				    ws->dtls_selected_comp->id) {
	...
	if (bandwidth_update(&ws->b_tx, dtls_to_send.size, tnow) != 0) {
		...
		if (DTLS_ACTIVE(ws)->udp_state == UP_ACTIVE) {
			...
			ret = dtls_send(DTLS_ACTIVE(ws), dtls_to_send.data + 7,
					dtls_to_send.size + 1);
			DTLS_FATAL_ERR_CMD(
				ret, exit_worker_reason(ws, REASON_ERROR));
		...
		if (DTLS_ACTIVE(ws)->udp_state != UP_ACTIVE || tls_retry != 0) {
			...
			ret = cstp_send(ws, cstp_to_send.data,
					cstp_to_send.size + 8);
			CSTP_FATAL_ERR_CMD(
				ws, ret, exit_worker_reason(ws, REASON_ERROR));
		}
...
static char *replace_vals(worker_st *ws, const char *txt)
{
	str_st str;
	int ret;
	str_rep_tab tab[3];
...
static int send_routes(worker_st *ws, struct http_req_st *req, char **routes,
		       unsigned int routes_size, bool include)
{
	unsigned int i;
	unsigned int ip6;
	const char *txt;
	...
	for (i = 0; i < routes_size; i++) {
		...
		if (ip6 != 0 && ws->full_ipv6) {
			ret = cstp_printf(ws, "X-CSTP-Split-%s-IP6: %s\r\n",
					  txt, routes[i]);
		} else {
			ret = cstp_printf(ws, "X-CSTP-Split-%s: %s\r\n", txt,
					  routes[i]);
		}
		if (ret < 0)
			return ret;
...
static void set_socket_timeout(worker_st *ws, int fd)
{
	...
	tval.tv_usec = 0;
	ret = setsockopt(fd, SOL_SOCKET, SO_RCVTIMEO, &tval, sizeof(tval));
	if (ret == -1) {
		int e = errno;
...
#define MAX_DTLS_CRYPTO_OVERHEAD                                          \
	(CSTP_DTLS_OVERHEAD + tls_get_overhead(GNUTLS_DTLS1_0,            \
					       GNUTLS_CIPHER_AES_128_CBC, \
					       GNUTLS_MAC_SHA1))
#define MAX_DTLS_PROTO_OVERHEAD(ws)                                    \
	((ws->proto == AF_INET) ? (IP_HEADER_SIZE + UDP_HEADER_SIZE) : \
...
static void calc_mtu_values(worker_st *ws)
{
	...
	if (DTLS_ACTIVE(ws)->udp_state != UP_DISABLED) {
		...
		oclog(ws, LOG_DEBUG, "DTLS overhead is %u",
...
static int connect_handler(worker_st *ws)
{
	...
	if (ws->user_config->hostname) {
		strlcpy(ws->req.hostname, ws->user_config->hostname,
	...
	if (ws->user_config->dpd > 0) {
		ret = cstp_printf(ws, "X-CSTP-DPD: %u\r\n",
				  ws->user_config->dpd);
		SEND_ERR(ret);
	}
	...
	if (DATA_MTU(ws, ws->link_mtu) < 1280 && ws->vinfo.ipv6 &&
	    req->no_ipv6 == 0) {
		oclog(ws, LOG_INFO,
		      "Connection MTU (link: %u, data: %u) is not sufficient for IPv6 (1280)",
		      ws->link_mtu, DATA_MTU(ws, ws->link_mtu));
		req->no_ipv6 = 1;
	}
	...
	if (ws->vinfo.ipv4 && req->no_ipv4 == 0) {
		...
		ret = cstp_printf(ws, "X-CSTP-Address: %s\r\n", ws->vinfo.ipv4);
		SEND_ERR(ret);

		if (ws->user_config->ipv4_netmask) {
			ret = cstp_printf(ws, "X-CSTP-Netmask: %s\r\n",
	...
	if (ws->vinfo.ipv6 && req->no_ipv6 == 0 &&
	    ws->user_config->ipv6_prefix != 0) {
		oclog(ws, LOG_NOTICE, "sending IPv6 %s/%u", ws->vinfo.ipv6,
		      ws->user_config->ipv6_subnet_prefix);
		if (ws->full_ipv6 && ws->user_config->ipv6_subnet_prefix) {
			ret = cstp_printf(ws, "X-CSTP-Address-IP6: %s/%u\r\n",
					  ws->vinfo.ipv6,
					  ws->user_config->ipv6_subnet_prefix);
			SEND_ERR(ret);
		} else {
			...
			ret = cstp_printf(ws, "X-CSTP-Netmask: %s/%u\r\n", net,
					  ws->user_config->ipv6_subnet_prefix);
			SEND_ERR(ret);
		}
	}
	...
	if (ws->full_ipv6 == 0) {
		req->no_ipv6 = 1;
		oclog(ws, LOG_INFO,
		      "IPv6 routes/DNS disabled because IPv6 support was not requested.");
	} else {
		switch (req->user_agent_type) {
		case AGENT_OPENCONNECT_V3:
			req->no_ipv6 = 1;
			oclog(ws, LOG_INFO,
			      "IPv6 routes/DNS disabled because the agent is known not to support them.");
			break;
		...
		case AGENT_ANYLINK:
			oclog(ws, LOG_DEBUG,
			      "Enabling IPv6 routes/DNS because the agent is known to support them.");
			break;
		case AGENT_UNKNOWN:
		default:
			oclog(ws, LOG_NOTICE,
	...
	for (i = 0; i < ws->user_config->n_dns; i++) {
		...
		if (req->user_agent_type == AGENT_ANYCONNECT) {
			...
			ret = cstp_printf(ws, "X-CSTP-DNS: %s\r\n",
					  ws->user_config->dns[i]);
		}
		SEND_ERR(ret);
	}
	...
	for (i = 0; i < ws->user_config->n_nbns; i++) {
		...
		ret = cstp_printf(ws, "X-CSTP-NBNS: %s\r\n",
	...
	for (i = 0; i < ws->user_config->n_split_dns; i++) {
		...
		ret = cstp_printf(ws, "X-CSTP-Split-DNS: %s\r\n",
				  ws->user_config->split_dns[i]);
		SEND_ERR(ret);
	}
	...
	if (ws->default_route == 0) {
		ret = send_routes(ws, req, ws->user_config->routes,
				  ws->user_config->n_routes, 1);
		SEND_ERR(ret);
	...
	ret = send_routes(ws, req, ws->user_config->no_routes,
			  ws->user_config->n_no_routes, 0);
	SEND_ERR(ret);

	ret = cstp_printf(ws, "X-CSTP-Keepalive: %u\r\n",
	...
	if (WSCONFIG(ws)->proxy_url != NULL) {
		...
		if (url != NULL) {
			ret = cstp_printf(
				ws, "X-CSTP-MSIE-Proxy-Pac-URL: %s\r\n", url);
			SEND_ERR(ret);
			talloc_free(url);
		}
	}
	...
	if (!ws->user_config->has_session_timeout_secs) {
		...
		time_t expiration = ws->session_start_time +
				    ws->user_config->session_timeout_secs;
	...
	if (DTLS_ACTIVE(ws)->udp_state != UP_DISABLED) {
		if (ws->user_config->dpd > 0) {
			ret = cstp_printf(ws, "X-DTLS-DPD: %u\r\n",
					  ws->user_config->dpd);
			SEND_ERR(ret);
		}
		...
		ret = cstp_printf(ws, "X-DTLS-Keepalive: %u\r\n",
				  ws->user_config->keepalive);
		SEND_ERR(ret);
		...
		if (ws->req.use_psk || !WSCONFIG(ws)->dtls_legacy) {
			...
			oclog(ws, LOG_INFO,
			      "DTLS ciphersuite: " DTLS_PROTO_INDICATOR);
			ret = cstp_printf(
				ws, "X-DTLS-CipherSuite: " DTLS_PROTO_INDICATOR
				    "\r\n");
		} else if (ws->req.selected_ciphersuite) {
			oclog(ws, LOG_INFO, "X-DTLS-Session-ID: %s",
			      ws->buffer);

			ret = cstp_printf(ws, "X-DTLS-Session-ID: %s\r\n",
					  ws->buffer);
			SEND_ERR(ret);
			...
			ret = cstp_printf(
				ws, "X-DTLS%s-CipherSuite: %s\r\n",
				(ws->req.selected_ciphersuite->dtls12_mode !=
				 0) ?
					"12" :
					"",
				ws->req.selected_ciphersuite->oc_name);
			...
			ret = cstp_printf(ws, "X-DTLS-MTU: %u\r\n",
					  DATA_MTU(ws, ws->link_mtu));
			SEND_ERR(ret);
			oclog(ws, LOG_INFO, "DTLS data MTU %u",
			      DATA_MTU(ws, ws->link_mtu));
		}
		SEND_ERR(ret);
	}
	...
	if (ws->buffer_size < ws->link_mtu + 16) {
		oclog(ws, LOG_ERR, "buffer size is smaller than MTU (%u < %u)",
...
exit:
	...
	if (DTLS_INACTIVE(ws)->udp_state == UP_ACTIVE &&
	    DTLS_INACTIVE(ws)->dtls_session) {
		      ...
		      time_t now, unsigned int is_dtls)
{
	...
	case AC_PKT_DPD_OUT:
		if (is_dtls == 0) {
			...
			if (ret < 0) {
				oclog(ws, LOG_ERR, "could not send data: %d",
				      ret);
				return -1;
			}
		} else {
			...
			if (buf_size - CSTP_DTLS_OVERHEAD >
				...
				data_mtu_set(ws, DTLS_ACTIVE(ws),
					     buf_size - CSTP_DTLS_OVERHEAD);
			}
			...
			if (ret < 0) {
				oclog(ws, LOG_TRANSFER_DEBUG,
				      "received DTLS DPD; error in sending response: %s",
				      gnutls_strerror(ret));
			} else {
				oclog(ws, LOG_TRANSFER_DEBUG,
	...
	case AC_PKT_COMPRESSED:
		...
		if (plain_size <= 0) {
			oclog(ws, LOG_ERR, "decompression error %d",
			      (int)plain_size);
			return -1;
		}
		plain = ws->decomp;
	...
	default:
		oclog(ws, LOG_DEBUG, "received unknown packet %u/size: %u",
		      (unsigned int)head, (unsigned int)buf_size);
	}
			   ...
			   time_t now)
{
	...
	if (buf[0] != 'S' || buf[1] != 'T' || buf[2] != 'F' || buf[3] != 1 ||
	    buf[7]) {
	...
	if (buf_size != 8 + pktlen) {
		oclog(ws, LOG_INFO,
		      "unexpected CSTP length (have %u, should be %d)",
		      (unsigned int)pktlen, (unsigned int)buf_size - 8);
		return -1;
	}
...
{
	if ((dtls->udp_state > UP_WAIT_FD) && (dtls->dtls_session != NULL) &&
